 ...
 *
 * Main window contains list of images on the backend and button for uploading
 * new images. Image list contains image thumbnail (generated by Enginio
 * backend) and some image metadata. Clicking list items downloads image file
 * and displays it in dialog window. Clicking the red x deletes images from
 * backend.
 *
 * In the backend images are represented as objects of type "objects.image". These
...
Rectangle {
    ...
    ListView {
        id: imageListView
        model: images // get the data from EnginioModel
    ...
    // Delegate for displaying individual rows of 'images' model
    Component {
    ...
    FileDialog {
        ...
        onSelectionAccepted: {
            ...
            reply.finished.connect(function() {
                ...
                uploadReply.finished.connect(function() { var tmp = images.query; images.query = {}; images.query = tmp; })
            })
